...
export default function PrunePage() {
  ...
  useEffect(() => {
    if (!modelId) return
    setError(null)
    analyzePrune(modelId).then(setResult).catch((err) => setError(err.message))
  }, [modelId])
    ...
    <ModelLayout modelId={modelId} activeTab="prune">
      <div style={page}>
        ...
        {result && (
          <>
            <div style={cards}>
              <Metric label="Layers" value={String(result.summary.total_prunable_layers)} />
              <Metric label="Params pct" value={`${result.summary.total_prunable_params_pct.toFixed(2)}%`} />
              <Metric label="Ratio" value={`${Math.round(result.summary.recommended_prune_ratio * 100)}%`} />
            </div>
            {result.layers.map((layer) => (
              <section key={layer.layer_name} style={panel}>
                <div style={title}>{layer.layer_name} · {layer.total_channels} channels</div>
